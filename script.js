function createNewCard() {
  ...
  let innerHtml = '<div class="card-down"></div><div class="card-up"></div>';
...
function createCards(parentElement, shuffledImageClasses) {
  ...
  for (let i = 0; i < 12; i++){
    let result = appendNewCard(parentElement);
    result.classList.add(shuffledImageClasses[i]);
    let obj = {
      index: i,
      element: result,
...
function onCardFlipped(newlyFlippedCard) {
  ...
  let flipCounter = document.getElementById("flip-count");
  incrementCounter("flips", flipCounter);
  ...
  if (doCardsMatch(lastCardFlipped, newlyFlippedCard) === false){
    let cardOne = lastCardFlipped.element;
    let cardTwo = newlyFlippedCard.element;
...
/* Step 6: Reset 'lastCardFlipped' to null */
  ...
  let cardOne = lastCardFlipped.element;
  let cardTwo = newlyFlippedCard.element;
  cardOne.classList.add("border-glow");
  cardTwo.classList.add("border-glow");
  if (matchCounter.innerHTML == 6){
    winAudio.play();
  } else{
    matchAudio.play();
  }
